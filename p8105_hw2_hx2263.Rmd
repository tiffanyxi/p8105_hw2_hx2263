...
brfss_prop = brfss%>%
  ...
  janitor::clean_names() %>%
  mutate(prop = (excellent + very_good )/100)
brfss_prop$prop[0:20]
``` 
...
```
* `r nrow(brfss_loco)` unique locations are included in the dataset. The number of states are `r nrow(brfss_state)`, so, yes, every state is represented. NJ(New Jersey) is observed the most.

```{r}
brfss_2002_excel = brfss %>% 
  filter(year == 2002 & response == "Excellent") %>% 
  filter(!is.na(data_value))
```

* In 2002, the median of the “Excellent” response value is `r median(brfss_2002_excel$data_value)`.


* Histogram of “Excellent” response values in the year 2002
```{r}
ggplot(brfss_2002_excel, aes(x = data_value)) + 
  geom_histogram()
```
